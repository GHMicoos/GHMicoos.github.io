...
2. 枚举总有一个基础类型，包括 `byte`、`sbyte`、`short`、`ushort`、`int`（默认）、`uint`、`long`或 `ulong`，  
但是可以使用`继承语法`来指定一个基础类型（注释，所以枚举的性能取决于基础类型的性能）
  ``` C#
...
4. 从**枚举转换成基础类型**，以及从**基础类型转换为枚举类型**，都必须**显示转换**，而不是隐式转换。
``` C#
   //eg:
   //基础类型转枚举
...
```
5. 在一个枚举中部插入一个枚举值，会使其后的所有枚举值发生顺移。
``` C#
//old
...
### 五 字符串与枚举值转换
1. **枚举值**转换为**枚举值标识符**。使用`ToString()`方法。
``` C#
...
```
2. **字符串**转换为**枚举** 使用：`Enum.Parse()`, Enum.TryParse() Enum.TryEnum<T>()
``` C#
string str = "Joined";
//Enum.Parse() 要抛异常 需处理
